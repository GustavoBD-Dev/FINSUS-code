...
-- Extensión de OpenFIN: saldos_rango_producto 
-- Saldos por rango de producto
-- 06/01/2023

-- ----------------------------------------------------------------------------
-- 06/01/2023 
-- Crea tablas realcionadas con esta extensión
CREATE OR REPLACE FUNCTION saldos_rango_producto___db ()
  RETURNS INTEGER AS $$
DECLARE
  -- Variables  
  _id       TEXT;
  _desc     TEXT;
  _fecha    TEXT;
  _version  TEXT := 'saldos_rango_producto.1';
...
-- 06/01/2023 
-- Inicialización
CREATE OR REPLACE FUNCTION saldos_rango_producto___ini ()
  RETURNS BOOLEAN AS $$
DECLARE
  -- Variables
...
SELECT of_db_drop_type('ofx_saldos_rango_producto','CASCADE');
CREATE TYPE ofx_saldos_rango_producto AS (
  cliente   TEXT,
  nombre    TEXT,
  cuenta    TEXT,
  saldo     NUMERIC
...
CREATE OR REPLACE FUNCTION saldos_rango_producto (var_producto_min INTEGER, var_producto_max INTEGER)
  RETURNS SETOF ofx_saldos_rango_producto AS $$
DECLARE
  r   ofx_saldos_rango_producto%ROWTYPE;
BEGIN
  FOR r IN 
...
-- 06/01/2023 
-- Finalización
CREATE OR REPLACE FUNCTION saldos_rango_producto___fin ()
  RETURNS BOOLEAN AS $$
DECLARE
  -- Variables
BEGIN -- (c) 2011 Servicios de Informática Colegiada, S.A. de C.V.
...
-- 06/01/2023 
-- Validciones
CREATE OR REPLACE FUNCTION saldos_rango_producto___val (p_variable TEXT, p_valor TEXT)
  RETURNS BOOLEAN AS $$
DECLARE
  -- Variables
BEGIN -- (c) 2011 Servicios de Informática Colegiada, S.A. de C.V.
...
CREATE OR REPLACE FUNCTION saldos_rango_producto___on_click (p_button TEXT, p_data TEXT) 
  RETURNS INTEGER AS $$
DECLARE
  -- Variables
BEGIN -- (c) 2011 Servicios de Informática Colegiada, S.A. de C.V.
...
--CREATE OR REPLACE FUNCTION saldos_rango_producto___cursor_changed (p_widget TEXT, p_path TEXT, p_row_info TEXT[])
--  RETURNS INTEGER AS $$
--DECLARE
--  -- Variables
--BEGIN -- (c) 2011 Servicios de Informática Colegiada, S.A. de C.V.
...
--CREATE OR REPLACE FUNCTION saldos_rango_producto___row_activated (p_widget TEXT, p_path TEXT, p_row_info TEXT[])
--  RETURNS INTEGER AS $$
--DECLARE
--  -- Variables
--BEGIN -- (c) 2011 Servicios de Informática Colegiada, S.A. de C.V.
