...
--
      ...
      LOOP
        ...
        --END IF;
      END LOOP;
      --IF (ra.periodo=202207) THEN
      --RAISE NOTICE '============== _deposito % _disposicionesm %, _depositopre %',_deposito,_disposicionesm,_depositopre;
      --END IF;
...
--     --   _saldo_periodo := _saldo_periodo + COALESCE(_pago4006,0.00);
     -- END IF;
      ...
      --RAISE NOTICE '_saldo_periodo %  _saldo_corte_ant %',_saldo_periodo,_saldo_corte_ant;
      _saldo_corte := (_saldo_periodo) + (_saldo_corte_ant) ;--- COALESCE(_retiros_sdof,0.00);
      --RAISE NOTICE 'saldo corte %',
      -- 04/mayo/2022
     ...
     -- SELECT INTO _sdoaf saldo FROM acreedores WHERE (idsucaux,idproducto,idauxiliar)=(_idsucauxref,_idproductoref,_idauxiliarref);
      ...
      _deposito         := numeric_larger(_deposito,0.00);
...
FOR rcreddet IN SELECT fecha,concepto,retiro,deposito,saldo,referencia
    ...
    IF (rctassdo.saldo>0.00) THEN
      ...
      IF (rctasp.tipocalculo=9970) THEN  
        ...
        SELECT INTO _ocdeuda saldo+interes_total+impuesto_total+costos_asociados AS deuda
         ...
         WHERE fecha=ps_afecha AND (idsucaux,idproducto,idauxiliar)=(rctassdo.idsucaux,rctassdo.idproducto,rctassdo.idauxiliar);
        rctassdosaldo := _ocdeuda;
        r.otros_creditos_mesanterior_lc  := r.otros_creditos_mesanterior_lc + to_char(COALESCE(_ocdeuda,0.00),'FM999,999,990.90')::TEXT;    
        ...
        r.otros_creditos_disposiciones_lc := r.otros_creditos_disposiciones_lc + to_char(COALESCE(_disposiciones,0.00),'FM999,999,990.90');
        r.otros_creditos_interes_lc       := r.otros_creditos_interes_lc + to_char(COALESCE(_interes,0.00),'FM999,999,990.90');
        ...
        _menscom         := COALESCE(_menscom,0.00);      
        _menscomdif      := COALESCE(_menscomdif,0.00);  
        r.otros_creditos_sdofinal_lc := r.otros_creditos_sdofinal_lc + to_char((COALESCE(_ocdeuda,0.00) + COALESCE(_ocmensualidad,0.00) + COALESCE(_menssegvida,0.00) + COALESCE(_menssegauto,0.00) +COALESCE(_mensgps,0.00) +
                                                                        COALESCE(_menscom,0.00) + COALESCE(_menscomdif,0.00)),'FM999,999,990.90');
        r.otros_creditos_capvigente_lc  := r.otros_creditos_capvigente_lc + to_char(COALESCE((rocc.saldo - rocc.montovencido),0.00),'FM999,999,990.90');
        r.otros_creditos_capvencido_lc  := r.otros_creditos_capvencido_lc + to_char(COALESCE(rocc.montovencido,0.00),'FM999,999,990.90');
        r.otros_creditos_intvenc_lc     := r.otros_creditos_intvenc_lc + to_char(COALESCE(rocc.interes_total,0.00),'FM999,999,990.90');
        r.otros_creditos_adeudo_lc := r.otros_creditos_adeudo_lc + to_char((COALESCE(_ocdeuda,0.00) + COALESCE(_ocmensualidad,0.00) + COALESCE(_menssegvida,0.00) + COALESCE(_menssegauto,0.00) +COALESCE(_mensgps,0.00)+
                                                                    COALESCE(_menscom,0.00) + COALESCE(_menscomdif,0.00)),'FM999,999,990.90');
        ...
        r.otros_creditos_proxabono_lc       := r.otros_creditos_proxabono_lc + to_char(COALESCE(_abonoppsig,0.00),'FM999,999,990.90');
        r.otros_creditos_proxinteres_lc     := r.otros_creditos_proxinteres_lc + to_char(COALESCE(_ioppsig,0.00),'FM999,999,990.90');
        r.otros_creditos_nopago_lc          := r.otros_creditos_nopago_lc + COALESCE(_idpagoppsig::TEXT,'0');
        ...
        r.otros_creditos_limite_lc        := r.otros_creditos_limite_lc + to_char(COALESCE(rctassdode.montoentregado,0.00),'FM999,999,990.90');

        IF (rctasp.tipocalculo=9970) THEN        
          _rctasvence := _venceppsig;
        ELSE
          SELECT INTO  _rctasvence vence
            FROM planpago 
           WHERE (idsucaux,idproducto,idauxiliar)=(rctassdo.idsucaux,rctassdo.idproducto,rctassdo.idauxiliar) AND 
  ...
  --RAISE NOTICE 'abono % interes % segvida % segunidad % gps % comision % comdif % desc % incre %',COALESCE(_credabonoppsig,0.00),COALESCE(_credioppsig,0.00),COALESCE(_credproxsegvida,0.00),COALESCE(_credproxsegunidad,0.00),COALESCE(_credproxgps,0.00),
  ...
  IF (of_iva_general(ps_idsucaux,ps_idproducto,ps_idauxiliar,_tipoprestamo,ps_afecha)) THEN
    _credproxmensualidad := _credproxmensualidad + COALESCE(round((round(_credioppsig,2)*round(factor_iva_io,2)),2),0.00);
  END IF;
  ...
  _adeudopendiente := to_char(of_numeric(r.monto_vencido_cap)+
                      (round(of_numeric(r.interes)-
                       COALESCE(_intdif,0),2))+
                       COALESCE(_segunidad,0.00)+
                       COALESCE(_segvida,0.00)+
                       COALESCE(_gps,0.00)+
                       COALESCE(_com,0.00)+
                       COALESCE(_comdif,0.00)+
                       COALESCE(_iodifnoex,0.00),'FM999,999,990.90');
  /*
  RAISE NOTICE 'Deuda: 
                   Monto vencido: %
                   Interes:       %
                   ...
                   comision dif   %
                   IO Dif No Exig %',
                   of_numeric(r.monto_vencido_cap),
                   of_numeric(r.interes),
                   COALESCE(_intdif,0),
                   COALESCE(_segunidad,0.00),
                   COALESCE(_segvida,0.00),
                   COALESCE(_gps,0.00),
  ...
  r.kv_data     := ARRAY['contrato_gazel'          , coalesce(rco.contrato_gazel::TEXT,''),
                         ...
                         'comdifactm'              , COALESCE(to_char(0.00,'FM999,999,990.90'),'0.00'),
...
BEGIN
  ...
  FOR r IN SELECT idsucaux,idproducto,idauxiliar,idsucursal,idrol,idasociado
            ...
            WHERE idproducto::TEXT = ANY (_productos_arr) AND
                  ...
                  (x_todos))
            AND idproducto<>5200 AND of_producto_subtipo(idproducto)='PRE' AND estatus=3  
           --       idproducto IN (3100,3102,3123,3124,3125,3200,3201,3205,3207,3209,3210,3211,3212,3213,3214,3300,3303,3307,3400,3401,3403,3405,3409,3500,3950)
            --WHERE (idsucaux,idproducto,idauxiliar) IN (SELECT idsucaux,idproducto,idauxiliar FROM diferencias )
